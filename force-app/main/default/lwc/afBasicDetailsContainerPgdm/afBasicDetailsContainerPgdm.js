...
export default class AfBasicDetailsContainerPgdm extends LightningElement {
    ...
    `
            ...
            personalDetails: {
                ...
                fields: [
                    { api: "First_Name__c", type: "text", span:3, label: "First Name", required: true, maxlength: '255' },
                    { api: "Middle_Name__c", type: "text", span:3, label: "Middle Name", maxlength: '255' },
                    { api: "Last_Name__c", type: "text", span:3, label: "Last Name", required: true, maxlength: '255', helpText:'If you do not have Last Name you may enter (.) dot' },

                    { api: "Gender__c", type: "picklist", span:3, label: "Gender", required: true },
                    { api: "Date_of_Birth_As_Per_10th_Marksheet__c", span:3, type: "date", min:"1970-1-1", label: "Date of Birth", required: true },
                    ...
                    { api: "HavePassport__c", type: "picklist", span:3, label: "Do you have a Passport?", required: true, visibleWhen: { "otherResources.showHavePassportField": true } },
                    { api: "PassportNumber__c", type: "text", span:3, label: "Passport Number", maxlength: '25', pattern:"^[A-Z0-9<]{3,20}$", required: true, visibleWhen: { "otherResources.showPassportField": true }, },
            ...
            correspondenceAddress: {
                ...
                rows: [
                    {
                        fluid: true,
                        fields: ["Corr_Country__c", "Corr_State__c", "Corr_City__c", "Other_Corr_City__c", "Corr_Ind_City__c", "Other_Corr_Ind_City__c"]
                    },
                    {
                        columns: [
                            { width: 6, fields: ["Corr_Address1__c"] },
                        ]
                    },
                    {
                        columns: [
                            ...
                            { width: 5, fields: ["Is_Permanent_Same__c"] }
                        ]
                    }
                ],
                fields: [
                    ...
                    { 
                        ...
                        sortInfo: ['Order__c DESC NULLS LAST'],
                        dynamicFilter: "corrIndCity",
                        allowOther: true,
                        visibleWhen: { "otherResources.showCorrIndCityField": true },
                    },
                    { 
                        api: "Other_Corr_Ind_City__c", 
                        type: "text", 
                        label: "Other City", 
                        span: 3,
                        required: true,
                        visibleWhen: { "otherResources.showOtherCorrIndCityField":true}, 
                        maxlength: '100',
                    },
                    
                    { api: "Corr_Address1__c", type: "text", label: "Address", required: true, maxlength:"32768" },
                    { 
                        api: "Corr_Pincode__c", 
                        type: "text", 
                        label: "Pincode", 
                    ...
                    },
                    {
                        ...
                        type: "radio",
                        span: 5,
                        label: "Is permanent address same as address for correspondence?",
                        options: [
                            { label: "Yes", value: "Yes" },
                            { label: "No", value: "No" }
                        ]
            ...
            permanentAddress: {
                ...
                rows: [
                    {
                        fluid: true,
                        fields: ["Perm_Country__c", "Perm_State__c", "Perm_City__c", "Other_Perm_City__c", "Perm_Ind_City__c", "Other_Perm_Ind_City__c"]
                    },
                    {
                        columns: [
                            { width: 6, fields: ["Perm_Address1__c"] },
                        ]
                    },
                    {
                        columns: [
                ...
                fields: [
                    ...
                    { 
                        ...
                        objectApi: "IndCity__c",
                        sortInfo: ['Order__c DESC NULLS LAST'],
                        dynamicFilter: "permIndCity",
                        allowOther: true,
                        visibleWhen: { "otherResources.showPermIndCityField": true },
                    },
                    { 
                        api: "Other_Perm_Ind_City__c", 
                        type: "text", 
                        label: "Other City", 
                        required: true,
                        span: 3,
                        visibleWhen: { "otherResources.showOtherPermIndCityField": true }, 
                        maxlength: '100',
                    },
    ...
    get showHavePassportField() {
        if(this.basic?.personalDetails?.Nationality__c=='Indian'){
            return true;
        }
    }
    ...
    get showAadhaarField() {
        const v = this.basic.personalDetails?.HaveAadhaar__c || '';
        return v === "Yes";
    }
    ...
    get showCorrIndCityField() {
        const addr = this.basic.correspondenceAddress || {};
        const isIndia = addr.Display?.Corr_Country__c === 'India';
        const cityIsOther = addr?.Display?.Corr_City__c === 'Other';
        // India only: show city dropdown once district is selected
        return isIndia && !!addr.Corr_City__c && !cityIsOther;
    }


    get showOtherCorrIndCityField() {
        const addr = this.basic.correspondenceAddress || {};
        const isIndia = addr.Display?.Corr_Country__c === 'India';
        const cityIsOther = addr?.Display?.Corr_City__c === 'Other';
        const indCityIsOther = addr?.Display?.Corr_Ind_City__c === 'Other';
        return isIndia && (cityIsOther || indCityIsOther);
    }
    ...
    get corrIndCity() {
        const addr = this.basic.correspondenceAddress;
        if (!addr?.Corr_City__c) return null;

        return {
            criteria: [
                {
                    fieldPath: 'District__c',
                    operator: 'eq',
                    value: addr.Corr_City__c
                },
                {
    ...
    get showPermIndCityField() {
        ...
        // India only: show city dropdown once district is selected
        const cityIsOther = perm?.Display?.Perm_City__c === 'Other';
        return isIndia && !!perm.Perm_City__c && !cityIsOther;
    }

    get showOtherPermIndCityField() {
        const perm = this.basic.permanentAddress || {};
        const isIndia = perm.Display?.Perm_Country__c === 'India';
        const permEnabled = this.basic.correspondenceAddress?.Is_Permanent_Same__c === 'No';
        if (!permEnabled) return false;
        const cityIsOther = perm?.Display?.Perm_City__c === 'Other';
        const indCityIsOther = perm?.Display?.Perm_Ind_City__c === 'Other';
        return isIndia && (cityIsOther || indCityIsOther);
    }
    ...
    get permIndCity() {
        const addr = this.basic.permanentAddress;
        if (!addr?.Perm_City__c) return null;
    ...
    reviseFieldConfigs() {
        ...
        const cityLabelMap = { true : 'District', false : 'City' };
        const otherCityLabelMap = { true : 'Other District', false : 'Other City' };
        /* ----------------------------
        CORRESPONDENCE
        ---------------------------- */
        ...
        this.metadata.correspondenceAddress = {
            ...
            fields: this.metadata.correspondenceAddress.fields.map(f => {
                if (!["Corr_State__c","Corr_City__c","Other_Corr_City__c","Corr_Pincode__c","Corr_Ind_Pincode__c"].includes(f.api)) {
                    return f;
                }
                const updated = { ...f };
                ...
                if (f.api === "Corr_City__c") {
                    updated.label = isCorrIndia ? cityLabelMap.true : cityLabelMap.false;   // 🚀 remove pattern completely
                }

                if (f.api === "Other_Corr_City__c") {
                    updated.label = isCorrIndia ? otherCityLabelMap.true : otherCityLabelMap.false;   // 🚀 remove pattern completely
                }
        ...
        this.metadata.permanentAddress = {
            ...
            fields: this.metadata.permanentAddress.fields.map(f => {
                if (!["Perm_State__c","Perm_City__c","Other_Perm_City__c","Perm_Pincode__c","Perm_Ind_Pincode__c"].includes(f.api)) {
                    return f;
                }
                const updated = { ...f };
                ...
                if (f.api === "Perm_City__c") {
                    updated.label = isPermIndia ? cityLabelMap.true : cityLabelMap.false;   // 🚀 remove pattern completely
                }

                if (f.api === "Other_Perm_City__c") {
                    updated.label = isPermIndia ? otherCityLabelMap.true : otherCityLabelMap.false;   // 🚀 remove pattern completely
                }
    ...
    _injectPicklists() {
        ...
        setOptions("personalDetails", "Marital_Status__c", toOptions(pick.Marital_Status__c));
        setOptions("personalDetails", "Category__c", toOptions(pick.Category__c));
        ...
        const moveToTop = (options, valueToTop) => {
            const index = options.findIndex(o => o.value === valueToTop);
            if (index > 0) {
        ...
        const nationalityOptions = moveToTop(
            toOptions(pick.Nationality__c),
            'Indian'
        );

        setOptions("personalDetails", "Nationality__c", nationalityOptions);
    ...
    get isReadOnly() {
        return AfBasicDetailsContainerPgdm.LOCKED_APPLICATION_STATUSES.includes(this.application?.Application_Status__c) &&
            !AfBasicDetailsContainerPgdm.UNLOCK_ASSIGNMENT_STATUSES.includes(this.application?.Assignment_Status__c);
    }
    ...
    async fetchForm(appId) {
        try {
            ...
            if (this.basic.personalDetails.Date_of_Birth_As_Per_10th_Marksheet__c) {
                this.basic.personalDetails.FormattedAge__c =
                    this._computeAge(
                        refDate,
    ...
    get contextBlock() {
        return {
            ...
            otherResources: {
                ...
                showAadhaarField:this.showAadhaarField,
                showIocField:this.showIocField,
                showSiiField:this.showSiiField,
                
                showCorrCityField:this.showCorrCityField,
                showOtherCorrCityField:this.showOtherCorrCityField,
                showCorrIndCityField:this.showCorrIndCityField,
                showOtherCorrIndCityField:this.showOtherCorrIndCityField,
                ...
                showPermIndCityField:this.showPermIndCityField,
                showOtherPermIndCityField:this.showOtherPermIndCityField,
    ...
    handleLookupSet(e){
        ...
        if(['Corr_Country__c', 'Corr_City__c', 'Corr_Ind_City__c', 'Corr_Ind_Pincode__c', 'Perm_Country__c', 'Perm_City__c', 'Perm_Ind_City__c', 'Perm_Ind_Pincode__c'].includes(api)){
            this.reviseFieldConfigs();
    ...
    handleSectionFieldChange(e) {
        ...
        if(api === "HavePassport__c") {
            if(this.basic[sectionKey][api] === 'No') {
                this.basic.personalDetails.PassportNumber__c = null;
            }
        }

        if(api === "HaveAadhaar__c") {
            if(this.basic[sectionKey][api] === 'No') {
        ...
        if(api === "Nationality__c") {
            if(this.basic[sectionKey][api] === 'Indian') {
                this.basic.personalDetails.Indian_Origin_Card_IOC__c = null;
                this.basic.personalDetails.StudyInIndiaRegNo__c = null;
            }
        }
        ...
        if (api === 'Corr_Country__c') {
            ...
            this.basic.correspondenceAddress.Corr_Ind_City__c = null;
            this.basic.correspondenceAddress.Other_Corr_City__c = null;
            this.basic.correspondenceAddress.Other_Corr_Ind_City__c = null;
            this.basic.correspondenceAddress.Display ||= {};
            this.basic.correspondenceAddress.Display.Corr_City__c = null;
            this.basic.correspondenceAddress.Display.Corr_Ind_City__c = null;
        ...
        if (api === 'Perm_Country__c') {
            ...
            this.basic.permanentAddress.Perm_Ind_City__c = null;
            this.basic.permanentAddress.Other_Perm_Ind_City__c = null;
            this.basic.permanentAddress.Display ||= {};
            this.basic.permanentAddress.Display.Perm_City__c = null;
            this.basic.permanentAddress.Display.Perm_Ind_City__c = null;
        ...
        if (api === "Is_Permanent_Same__c") {
            ...
            if (normalized === "Yes") {
                ...
                ["Country", "State", "City", "Ind_City", "Address1", "Address2", "Pincode", "Ind_Pincode"]
                    .forEach(k => {
                ...
                this.basic.permanentAddress.Other_Perm_City__c = 
                    this.basic.correspondenceAddress.Other_Corr_City__c ?? null;
                this.basic.permanentAddress.Other_Perm_Ind_City__c = 
                    this.basic.correspondenceAddress.Other_Corr_Ind_City__c ?? null;                
            }
    ...
    _computeAge(refDateStr, dobStr) {
        try {
            ...
            if (d < 0) {
                m--;
                const prevMonth =
                    ref.getMonth() === 0 ? 11 : ref.getMonth() - 1;
                const prevYear =
                    ref.getMonth() === 0 ? ref.getFullYear() - 1 : ref.getFullYear();

                d += this._getLastDateOfMonth(prevMonth, prevYear);
            }
    ...
    async validateAll() {
        ...
        const personal = this.basic?.personalDetails || {};

        if (personal.Nationality__c === 'Indian') {
            const havePassport = personal.HavePassport__c;
            const haveAadhaar = personal.HaveAadhaar__c;

            if (havePassport === 'No' && haveAadhaar === 'No') {
                errors.personalDetails ||= {};

                errors.personalDetails.HavePassport__c =
                    'You must have either a Passport or Aadhaar';

                errors.personalDetails.HaveAadhaar__c =
                    'You must have either a Passport or Aadhaar';
            }
        }

        const actualAge = parseInt(this.basic?.personalDetails?.FormattedAge__c, 10) || 0;
